...
- **白名单限定，节省 Token** — 编译期通过 `HIL_WHITELIST` 标记可交互变量，AI 只在该组变量域内读写。拒绝全量扫 `.map` 文件，从根源避免大模型的 Token 浪费与上下文污染。
- **参数热重载，无需重烧** — 依托 32 位总线，在 50MHz 主频下约 20ns 即可完成一次参数的原子覆写。设备全速运行中修改，无需复位即可瞬间生效。
- **RTT 数据流，极低侵入** — 采用 SEGGER RTT 替代传统 UART。MCU 仅需向环形缓冲区内做内存拷贝即可完成数据上传，不占用外设引脚。对实时链路的侵入可控，无需为通信分配独立串口。
...
- 支持 MCP Tool 的 AI 助手（Claude Code、Cline、Codex）
